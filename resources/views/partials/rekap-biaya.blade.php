...
<body>
  <div class="container">
    ...
    <div class="table-wrapper" id="tableWrapper">
      ...
      <form method="POST" action="/rekap-biaya/simpan">
        ...
        <button onclick="exportToCSV()" class="submit-btn" style="margin-left: 10px; background-color: green;">Export ke CSV</button>
      </form>
    </div>
  </div>
  
  <script>
    // Format input angka menjadi format rupiah dengan titik pemisah ribuan
  ...
  // Fungsi update total biaya kesehatan otomatis
  function updateTotal(changedInput) {
    const row = changedInput.closest('tr');
    const inputs = row.querySelectorAll('input[type="number"]:not([name$="[total]"])');
    let total = 0;
    inputs.forEach(input => {
      let raw = input.dataset.raw || input.value.replace(/[^\d]/g, '');
      if (raw) total += parseInt(raw);
    });
  ...
  function exportToCSV() {
    ...
    rows.forEach(row => {
      ...
      cols.forEach(col => {
        ...
        if (input) {
          rowData.push((input.dataset.raw || input.value || '').replace(/\./g, ''));
        } else {
    ...
    link.setAttribute("download", "rekap_biaya_kesehatan.csv");
